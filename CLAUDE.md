...
| 엑셀 생성 (발주서) | ExcelJS | 발주서 템플릿 기반 쓰기 |
| 엑셀 생성 (운송장) | JSZip | 시트 XML 직접 편집 — ExcelJS 재직렬화 시 양식 손상 방지 |
| 테스트 | Vitest | 파서, 매칭엔진 등 핵심 로직만 |
| 알림 | Sonner | toast 알림 |
| 이모지 처리 | emoji-regex | 상품명 정규화 시 이모지 제거 |
| 아이콘 | Lucide React | |
| 배포 | Vercel | 무료 플랜 |
...
### 매칭키 (운송장 매칭 기준)
- **쿠팡**: `matchingKey = 묶음배송번호:옵션ID` (한 묶음배송번호에 여러 상품이 존재할 수 있음)
- 기존 쿠팡 묶음배송번호 운송장은 후보가 하나일 때만 매칭. 추가 업로드 시 기존 주문의 raw 묶음배송번호/옵션ID로 중복 비교.
...
- **`platformProductName`**: 플랫폼 원본 상품명 — 쿠팡: 노출상품명(옵션명)[col12], 토스: 상품명+옵션명 합침
- 양식 설정에서 둘 중 하나를 선택 (OR 관계 필수 검증)
- B업체는 `platformProductName` 사용, A업체는 `supplierProductName` 사용
...
### 토스 엑셀 구조
- 헤더 위치가 변동됨 → **동적 감지** (0~9행 스캔, 주문일시/주문번호/주문상품번호 마커)
- 일반적: 1행 그룹헤더, 2행 컬럼헤더, 3행 수정가능여부, 4행~ 데이터
- 2026-05-12 이후: 안내문구 1행 추가 → 헤더가 3행으로 이동
- 데이터 시작 = 헤더행 + 2

### 기타 규칙
- **보내는분 정보**: 발주서에서 항상 빈 칸
- **중복 주문 감지**: `platform + matchingKey` 기준, DB unique 제약으로 강제
- **기존 주문에 추가**: `append_orders_to_import` RPC로 중복 확인·새 주문 INSERT·집계를 한 트랜잭션에 처리. 기존 주문 ID/배정/운송장/파일 라벨 보존. 여러 파일이면 추가 대상 선택 필수.
- **대체하기**: 같은 work_session + 같은 platform → 기존 데이터 cascade 삭제 후 새로 생성 (확인 모달 필수)
...
> ✅ Claude Design으로 전체 화면 시안을 만든 뒤 디자인 시스템을 추출 완료.

### 참조 문서
- **디자인 토큰 + 컴포넌트 패턴**: `docs/REF_디자인_시스템.md` (단일 진실 공급원)
- **시안 원본**: `docs/design/` (HTML/CSS/JSX 프로토타입 43개 파일)
- **CSS 토큰**: `src/index.css`의 `@theme` 블록

### UI 구현 시 규칙
- `docs/REF_디자인_시스템.md`의 토큰과 컴포넌트 패턴을 따른다
- 시안 원본(`docs/design/`)의 HTML/CSS를 직접 읽고 픽셀 단위로 재현한다
- 시안에 빠진 화면이나 상태는 기존 시안의 톤/패턴과 일관되게 채운다
- 시안은 프로토타입이므로 구조를 복사하지 말고 React 컴포넌트로 재구현한다

### 브랜드
- **상호**: WithLab
- **방향**: 토스 스타일 미니멀 — 화이트 + 그레이 톤, 색상은 의미 있을 때만
- **타깃**: PC Chrome 전용, 1440px 최적 (최소 1024px)
- **폰트**: Pretendard Variable + JetBrains Mono (모노)
...
| 로그인 | `login.css/jsx` |
| 대시보드 | `home.css/jsx` |
| 발주서 - 주문 업로드 | `app.jsx` + `styles.css` |
| 발주서 - 공급처 배정 | `assign.css/jsx` |
| 발주서 - 발주서 다운로드 | `download.css/jsx` |
| 운송장 - 업로드 | `tracking-upload.css/jsx` |
| 운송장 - 매칭 결과 | `tracking.css/jsx` |
| 운송장 - 플랫폼 다운로드 | `tracking-download.css/jsx` |
| 매핑 - 품목↔공급처 | `mapping.css/jsx` |
| 매핑 - 공급처 관리 | `supplier.css/jsx` |
| 매핑 - 상품명 변환 | `mapping-rename.css/jsx` |
| 매핑 - 택배사 | `mapping-courier.css/jsx` |
| 양식 - 공급처 발주서 | `form-mgmt.css/jsx` |
| 양식 - 플랫폼 운송장 | `platform-form.css/jsx` |
