...
| 2026-08-06 | 운영 | Supabase 프로젝트만 인수자 조직으로 이관 (Vercel·GitHub은 본계정 유지) | Vercel Hobby는 멤버 초대를 지원하지 않아 프로젝트 전송이 불가. 유일한 우회로인 GitHub 로그인 연결 이전은 같은 계정의 다른 프로젝트(chuldu-hongdae 등) 자동배포를 끊음. 실질 자산(DB·Storage·Auth)은 전부 Supabase에 있어 이것만 넘겨도 목적 달성 |
| 2026-08-06 | 운영 | 이관 방식으로 프로젝트 전송(Transfer) 선택               | 전송은 인프라를 이동하지 않아 project ref·URL·anon key가 그대로 유지됨. 코드·환경변수·재배포 변경 0건. 신규 프로젝트 생성 후 덤프 복원 방식 대비 리스크가 훨씬 낮음                    |
| 2026-08-06 | 운영 | 대시보드 뷰의 `anon` 권한 회수                           | anon key만으로 작업건명·일별 주문 수·거래 공급처 수가 조회되고 있었음(실측 확인). 원인은 `20260510_grant_table_permissions.sql` 의 `ALTER DEFAULT PRIVILEGES ... TO anon` 이 이후 생성된 뷰에 자동 적용된 것 |
| 2026-08-06 | 운영 | `security_invoker = on` 은 적용하지 않기로 결정          | 적용 시 대시보드가 `57014 statement timeout` 으로 다운. 뷰의 교차곱이 원인이며, RLS 정책이 `to authenticated using (true)` 라 실질 보안 이득도 없음. 뷰 재작성 후 재검토 |
| 2026-08-06 | 운영 | 대시보드 뷰를 스칼라 서브쿼리 방식으로 재작성 후 적용    | `orders` × `trackings` 교차곱 제거. 3,023ms → 14.5ms. `allocations` 에 `work_session_id` 가 직접 있어 `orders` 경유가 불필요했음. 적용 전 41개 작업건 집계값 전수 대조로 동등성 확인(불일치 0행) |
...
| 등록일 | 단계 | 이슈 | 상태 | 해결일 |
| ------ | ---- | ---- | ---- | ------ |
| 2026-08-06 | 운영 | 대시보드 뷰가 `orders` × `trackings` 교차곱을 생성 (작업건당 약 35만 행) → 조회에 3,023ms 소요. 대시보드는 이 뷰 1개 + 통계 7개를 동시에 던지므로 `authenticated` 제한 시간 8초를 넘겨 500 발생. `20260806_rewrite_dashboard_view.sql` 로 스칼라 서브쿼리 전환하여 **14.5ms (약 200배)** 로 개선. 적용 전 41개 작업건 집계값을 전수 대조해 불일치 0행 확인 | 해결됨 | 2026-08-06 |
| 2026-08-06 | 운영 | `ALTER DEFAULT PRIVILEGES ... TO anon` 이 살아 있어, public 스키마에 새 뷰를 만들면 anon 노출이 재발함. 뷰 추가 시마다 `revoke` 필요 | 미해결 | - |
| 2026-08-06 | 운영 | Vercel Hobby 플랜은 상업적 사용을 금지 (Fair Use Guidelines). 사내 업무 시스템이므로 약관상 Pro 플랜 대상. 사용량 자체는 한도의 1% 미만이라 소진 위험은 없음 | 미해결 | - |
