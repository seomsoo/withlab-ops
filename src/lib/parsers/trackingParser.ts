...
import type { ParsedTracking, InvalidRow, TrackingParseResult } from '@/types'
...
export function parseTracking(workbook: WorkBook): TrackingParseResult {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    if (rawOrderKey === '') {
      invalidRows.push({ rowNumber: excelRowNumber, reason: '주문번호 누락', rawData })
      continue
    }
    if (trackingNumber === '') {
