...
## 플랫폼 감지 로직
- Delivery 우선, 없으면 전체 시트 1~10행에 `번호`, `묶음배송번호`, `주문번호` → 쿠팡
- 주문내역 시트 1~10행에 `주문일시`, `주문번호`, `주문상품번호` → 토스
- 1행에 `업체주문번호` → A업체 운송장
- 1행에 `거래처주문번호` → B업체 운송장

## 토스 파싱 주의사항
- 헤더 위치 **동적 감지** (0~9행 스캔, 주문일시/주문번호/주문상품번호 마커)
- 데이터 시작 = 헤더행 + 2 (헤더 다음 행은 수정가능여부 → 스킵)
