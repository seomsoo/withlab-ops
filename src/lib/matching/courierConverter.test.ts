...
describe('convertCourierName', () => {
  ...
  it('sourceSupplierId가 다르면 매핑 안 됨', () => {
    const result = convertCourierName(
      'CJ대한통운',
      'other-supplier',
      'coupang',
  ...
  it('같은 공급처의 여러 택배사 매핑 중 정확한 것만 선택', () => {
    const mappings = [
    ...
    const result = convertCourierName('한진택배', 'supplier-1', 'coupang', mappings)
    expect(result.name).toBe('한진')
    expect(result.isMapped).toBe(true)
  })

  it('빈 매핑 목록이면 원본 반환', () => {
    const result = convertCourierName('CJ대한통운', 'supplier-1', 'coupang', [])
